...
void RedisClient::DefaultTransporter::initSocket()
{
    ...
    connect(m_socket.data(), &QSslSocket::errorOccurred, this, &DefaultTransporter::error);
    connect(m_socket.data(), &QSslSocket::sslErrors, this, &DefaultTransporter::sslError);
    connect(m_socket.data(), &QAbstractSocket::readyRead, this, &AbstractTransporter::readyRead);
    connect(m_socket.data(), &QSslSocket::encrypted, this, [this]() {
...
bool RedisClient::DefaultTransporter::connectToHost()
{
    ...
    if (conf.useSsl()) {
        if (!QSslSocket::supportsSsl()) {
            emit errorOccurred(QString("SSL Error: Openssl is missing. Please install Openssl (%1)")
        ...
        if (!trustedCas.empty()) {
            m_socket->setLocalCertificateChain(trustedCas);
        }
        ...
        socketWaiter.addAbortSignal(m_socket.data(), &QSslSocket::errorOccurred);
        socketWaiter.addAbortSignal(m_connection, &RedisClient::Connection::shutdownStart);
        socketWaiter.addAbortSignal(m_socket.data(), &QSslSocket::disconnected);
        ...
        socketWaiter.addAbortSignal(m_socket.data(), &QSslSocket::disconnected);
        socketWaiter.addSuccessSignal(m_socket.data(), &QSslSocket::connected);
...
{
    if (error == QAbstractSocket::UnknownSocketError && m_runningCommands.size() > 0) {
    ...
    emit errorOccurred(QString("Connection error: %1").arg(m_socket->errorString()));
}
