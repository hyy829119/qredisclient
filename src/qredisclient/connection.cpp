...
bool RedisClient::Connection::connect(bool wait)
{
    ...
    QObject::connect(m_transporterThread.data(),
                     &QThread::started,
                     m_transporter.data(),
                     &AbstractTransporter::init);
    QObject::connect(m_transporterThread.data(),
                     &QThread::finished,
                     m_transporter.data(),
                     &AbstractTransporter::disconnectFromHost);
    QObject::connect(this,
                     &Connection::shutdownStart,
                     m_transporter.data(),
                     &AbstractTransporter::disconnectFromHost);
    QObject::connect(m_transporter.data(), &AbstractTransporter::connected, this, &Connection::auth);
    QObject::connect(m_transporter.data(),
    ...
    QObject::connect(this, &Connection::authError, this, [this](const QString &) { disconnect(); });
...
QFuture<RedisClient::Response> RedisClient::Connection::command(const RedisClient::Command &cmd)
{
    try {
...
QFuture<RedisClient::Response> RedisClient::Connection::command(QList<QByteArray> rawCmd, int db)
{
...
void RedisClient::Connection::pipelinedCmd(
    const QList<QList<QByteArray>> &rawCmds,
    QObject *owner,
    int db,
...
QFuture<RedisClient::Response> RedisClient::Connection::runCommand(const Command &cmd)
{
    if (!cmd.isValid())
    ...
    if (cmd.getOwner() && cmd.getOwner() != this)
        QObject::connect(cmd.getOwner(),
                         SIGNAL(destroyed(QObject *)),
                         m_transporter.data(),
                         SLOT(cancelCommands(QObject *)),
...
QSharedPointer<RedisClient::Connection> RedisClient::Connection::clone(bool copyServerInfo) const
{
    auto config = getConfig();
    config.setId(config.id());

    auto newConnection = QSharedPointer<RedisClient::Connection>(new RedisClient::Connection(config),
                                                                 &QObject::deleteLater);
...
void RedisClient::Connection::retrieveCollection(const ScanCommand &cmd,
...
void RedisClient::Connection::retrieveCollectionIncrementally(
    const ScanCommand &cmd, RedisClient::Connection::IncrementalCollectionCallback callback)
{
    if (!cmd.isValidScanCommand())
        throw Exception("Invalid command");
                                             ...
                                             long scanLimit)
{
    ...
    m_collectClusterNodeKeys = [this, result, callback, onConnect](const RawKeysList &res,
                                                                   const QString &err) {
        if (!err.isEmpty()) {
    ...
    getMasterNodes([this, callback, onConnect](const HostList &hosts, const QString &err) {
        ...
        m_notVisitedMasterNodes = QSharedPointer<HostList>(new HostList(hosts));

        clusterConnectToNextMasterNode(onConnect);
    });
}

void RedisClient::Connection::flushDbKeys(int dbIndex, std::function<void(const QString &)> callback)
{
    if (mode() == Mode::Cluster) {
        ...
        m_cmdCallback = [this, callback, dbIndex, onConnect](const RedisClient::Response &,
                                                             const QString &error) {
            if (!error.isEmpty()) {
        ...
        command(
            {"FLUSHDB"},
            this,
            [dbIndex, callback](const RedisClient::Response &, const QString &error) {
                if (!error.isEmpty()) {
                    callback(
                        QString(QObject::tr("Cannot flush db (%1): %2")).arg(dbIndex).arg(error));
                } else {
                    callback(QString());
                }
            },
...
void RedisClient::Connection::getDatabaseKeys(RawKeysListCallback callback,
                                              ...
                                              long scanLimit)
{
    QList<QByteArray> rawCmd{"scan",
                             "0",
                             "MATCH",
                             pattern.toUtf8(),
                             "COUNT",
                             QString::number(scanLimit).toLatin1()};
    ...
    int dbIndex)
{
    ...
    QList<QByteArray> rawCmd{"eval", LUA_SCRIPT, "0", nsSeparator.toUtf8(), filter.toUtf8()};
    ...
    evalCmd.setCallBack(this, [callback](RedisClient::Response r, QString error) {
        ...
        while (i != rootNamespaces.constEnd()) {
            rootNs.append(QPair<QByteArray, ulong>(i.key().toUtf8(), (ulong) i.value().toDouble()));
            ++i;
        }
                                                 ...
                                                 bool incrementalProcessing)
{
    ...
    cmdWithCallback.setCallBack(
        this,
        [this, cmd, result, callback, incrementalProcessing](RedisClient::Response r,
                                                             QString error) {
            if (r.isErrorMessage()) {
                /*
...
void RedisClient::Connection::callAfterConnect(std::function<void(const QString &err)> callback)
{
    ...
    QObject::connect(this, &Connection::error, context, [callback, context](const QString &err) {
        callback(err);
        context->deleteLater();
    });
}
    ...
    std::function<void(QVariantList, const QString &)> callback)
{
    ...
    cmd(
        ...
        this,
        -1,
        [callback](const Response &r) { callback(r.value().toList(), QString()); },
        [callback](const QString &err) {
            return callback(QVariantList(), QString("Cannot retrive nodes list: %1").arg(err));
        },
        true);
}

void RedisClient::Connection::getMasterNodes(
    std::function<void(RedisClient::Connection::HostList, const QString &)> callback)
{
...
void RedisClient::Connection::getClusterSlots(
    std::function<void(RedisClient::Connection::ClusterSlots, const QString &)> callback)
{
    rawClusterSlots([callback](QVariantList slotsList, const QString &err) {
        if (err.size() > 0 || slotsList.size() == 0) {
        ...
        foreach (QVariant clusterSlot, slotsList) {
            ...
            hashSlots.insert(r, {masterDetails[0].toString(), masterDetails[1].toInt()});
        }
...
RedisClient::Connection::Host RedisClient::Connection::getClusterHost(const Command &cmd)
{
...
QFuture<bool> RedisClient::Connection::isCommandSupported(QList<QByteArray> rawCmd)
{
    auto d = QSharedPointer<AsyncFuture::Deferred<bool>>(new AsyncFuture::Deferred<bool>());

    cmd(
        rawCmd,
        this,
        -1,
        [d](RedisClient::Response r) { d->complete(!r.isDisabledCommandErrorMessage()); },
        [d](const QString &err) { d->complete(!err.contains("unknown command")); });
...
void RedisClient::Connection::auth()
{
    ...
    auto testConnection = [this, handleConnectionError]() {
        cmd(
            ...
            [this](const Response &resp) {
                ...
                bool connectionWithPopulatedServerInfo = (m_serverInfo.parsed.size() > 0
                ...
                refreshServerInfo([this]() {
                    if (m_serverInfo.clusterMode) {
                        m_currentMode = Mode::Cluster;
                        getClusterSlots([this](const ClusterSlots &cs, const QString &err) {
                            if (err.size() > 0) {
                                emit error(QString("Cannot retrieve cluster slots: %1").arg(err));
                                return;
                            }
    ...
    if (m_config.useAuth() || m_config.useAcl()) {
        ...
        if (m_config.useAcl()) {
            authCmd = {"AUTH", m_config.username().toUtf8(), m_config.auth().toUtf8()};
        } else {
        ...
        command(
            authCmd,
            this,
            [handleAuthResp, handleConnectionError](RedisClient::Response r, QString err) {
                if (err.size() > 0)
...
RedisClient::ServerInfo RedisClient::ServerInfo::fromString(const QString &info)
{
    ...
    result.version = (versionRegex.indexIn(info) == -1) ? 0.0 : versionRegex.cap(1).toDouble();
